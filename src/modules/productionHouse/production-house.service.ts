...
import { ProductionHouse, ProductionHouseDocument } from './production-house.schema';
...
export class ProductionHouseService {
  ...
  async createProductionHouse(data: CreateProductionHouseDto): Promise<ProductionHouse> {
  ...
  async updateProductionHouse(id: string, data: UpdateProductionHouseDto): Promise<ProductionHouse> {
    const updated = await this.productionHouseModel.findByIdAndUpdate(id, data, { new: true }).exec();
    if (!updated) throw new NotFoundException('Production house not found');
    return updated;
  }

  async deleteProductionHouse(id: string): Promise<void> {
    const deleted = await this.productionHouseModel.findByIdAndDelete(id).exec();
